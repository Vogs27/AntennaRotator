...
#define MICROSTEP          16     ///< Set Microstep
#define MIN_PULSE_WIDTH    1    ///< In microsecond for AccelStepper
#define MAX_SPEED          6400  ///< In steps/s, consider the microstep
#define MAX_ACCELERATION   3200  ///< In steps/s^2, consider the microstep
#define SPR                3200 ///< Step Per Revolution, consider the microstep 200steps(1.8 deg)*16microsteos
...
#define AZ_DRIVER_ADDRESS 0b00 // TMC2209 Driver address according to MS1 and MS2
#define EL_DRIVER_ADDRESS 0b01 // TMC2209 Driver address according to MS1 and MS2
#define R_SENSE 0.11f // Match to your driver


easycomm comm;
TMC2209Stepper driver_az(&Serial1, R_SENSE, AZ_DRIVER_ADDRESS);
TMC2209Stepper driver_el(&Serial1, R_SENSE, EL_DRIVER_ADDRESS);
AccelStepper stepper_az(1, M1IN1, M1IN2);
AccelStepper stepper_el(1, M2IN1, M2IN2); //stepper.DRIVER
...
void setup() {
  ...
  comm.easycomm_init();
  Serial1.begin(19200);//Enable talking to tmc drivers

  driver_az.begin();             // Initiate pins and registeries
  driver_az.toff(5);                 // Enables driver in software
  driver_az.rms_current(1700, 0.5);    // Set stepper current to 600mA. The command is the same as command TMC2130.setCurrent(600, 0.11, 0.5);
  driver_az.pwm_autoscale(true);
  driver_az.microsteps(MICROSTEP);
  driver_az.intpol(true); //Interpolate to 256microsteps
  driver_az.en_spreadCycle(true);

  driver_el.begin();             // Initiate pins and registeries
  driver_el.toff(5);                 // Enables driver in software
  driver_el.rms_current(1700, 0.5);    // Set stepper current to 600mA. The command is the same as command TMC2130.setCurrent(600, 0.11, 0.5);
  driver_el.pwm_autoscale(true);
  driver_el.microsteps(MICROSTEP);
  driver_el.intpol(true); //Interpolate to 256microsteps
  driver_el.en_spreadCycle(true);
